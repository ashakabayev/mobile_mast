...
class _CollegeDetailScreenState extends State<CollegeDetailScreen> {
  ...
  final snackBar = SnackBar(content: Text('Повторите попытку позже!'));
  bool visib = true;
  @override
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: BlocProvider<CollegeDetailBloc>(
          ...
          child: BlocConsumer<CollegeDetailBloc, CollegeDetailState>(
            listener: (context, state) {
              if (state is CollegeDetailErrorState) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    backgroundColor: Colors.red,
                    elevation: 0,
                    content: Text("Повторите попытку позже."),
                  ),
                );
              }
            },
            builder: (context, state) {
              if (state is CollegeDetailLoadedState) {
                ...
                return /*alertOfDetailCollege()*/Center(
                  child: Text('Ошибка!'),
                );
              } else {
                return Center(
